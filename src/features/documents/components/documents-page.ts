...
export function renderDocumentsPage(
  ...
  renderAppShell(
    ...
    `<main id="main-content" class="record-main documents-main">
      <section class="record-heading documents-heading" aria-labelledby="documents-title"><div><p class="eyebrow">Encrypted local files</p><h1 id="documents-title">Documents</h1><p>Keep each person’s supporting files separate and available on this device.</p></div><div class="record-heading-actions single-action"><button id="add-document" class="primary-button compact-button" type="button"><span aria-hidden="true">＋</span> Add document</button></div></section>
      <section class="documents-profile-picker" aria-label="Choose a profile for documents">${renderPersonSwitcherMarkup()}</section>
      <section class="document-summary-grid" aria-label="Local document summary"><article class="document-summary-card"><p class="eyebrow">Selected profile</p><p class="document-summary-value"><span id="document-count">${documents.length}</span><small>${documents.length === 1 ? "document" : "documents"}</small></p><p>Up to 25 encrypted files per person</p></article><article class="document-summary-card"><p class="eyebrow">Document storage</p><p class="document-summary-date">${formatDocumentBytes(totalBytes)} <small>of 50 MB</small></p><div class="document-storage-track" role="progressbar" aria-label="Document storage used" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${Math.round(storagePercent)}"><span style="width: ${storagePercent}%"></span></div><p>Across every household profile on this device</p></article></section>
      <aside class="notice compact-notice documents-backup-notice" aria-labelledby="documents-backup-title"><span class="notice-icon" aria-hidden="true">i</span><div><h2 id="documents-backup-title">Keep the original files safely</h2><p>Document files are encrypted locally but are not yet included in UrbanFox JSON backups. Combined PDF export and document backup are planned next.</p></div></aside>
