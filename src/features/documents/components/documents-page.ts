...
<section class="vault-download-panel${members.length > 1 ? " has-household-download" : ""}"><button id="download-document-bundle" class="vault-download-button" type="button" ${documents.length === 0 && addressHistory.length === 0 ? "disabled" : ""}><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M3.5 6.5h6l2 2h9v10.5h-17Z"/><rect x="10" y="12.5" width="7" height="5.5" rx="1"/><path d="M11.5 12.5v-1a2 2 0 0 1 4 0v1"/></svg><span data-vault-download-label>DOWNLOAD MY ZIP</span></button>${members.length > 1 ? '<button id="download-household-document-bundle" class="vault-download-button vault-download-button-secondary" type="button"><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M4 8.5h16v11H4Z"/><path d="M8 8.5V6.8A2.8 2.8 0 0 1 10.8 4h2.4A2.8 2.8 0 0 1 16 6.8v1.7"/><path d="M8 13h8M12 10v6"/></svg><span data-household-vault-download-label>DOWNLOAD HOUSEHOLD ZIP</span></button>' : ""}</section>
