...
import { type EmploymentRecord, type SponsorshipStatus } from "../domain/employment";
...
export function renderEmploymentDialog(): string {
  return renderLiquidGlassDialog({
    ...
    iconSvg: '<svg viewBox="0 0 24 24"><path d="M5 7h14v12H5Z"/><path d="M9 7V5h6v2M8 11h8M8 15h5"/></svg>',
    ...
    actions: '<button class="primary-button liquid-dialog-save" type="submit">Save employment details</button>',
...
export function showEmploymentForm(root: HTMLElement, record: EmploymentRecord | null, letterStatus: string): void {
  ...
  form.reset();
  if (record) {
    (form.elements.namedItem("employerName") as HTMLInputElement).value = record.employerName;
    (form.elements.namedItem("jobTitle") as HTMLInputElement).value = record.jobTitle;
    (form.elements.namedItem("sponsorshipStatus") as HTMLSelectElement).value = record.sponsorshipStatus;
    (form.elements.namedItem("employmentStartDate") as HTMLInputElement).value = record.employmentStartDate;
    (form.elements.namedItem("employmentEndDate") as HTMLInputElement).value = record.employmentEndDate;
    (form.elements.namedItem("annualSalary") as HTMLInputElement).value = record.annualSalary === null ? "" : String(record.annualSalary);
  }
...
export function readEmploymentInput(form: HTMLFormElement): {
  employerName: string; jobTitle: string; sponsorshipStatus: SponsorshipStatus;
  employmentStartDate: string; employmentEndDate: string; annualSalary: number | null;
} {
  ...
  return {
    ...
    sponsorshipStatus: String(data.get("sponsorshipStatus") ?? "") as SponsorshipStatus,
