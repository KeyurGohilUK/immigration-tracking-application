...
export async function getEmploymentRecord(
  ...
  if (!isEmploymentCollection(value, profileId))
    throw new Error("Decrypted employment data is invalid.");
  return value[0] ?? null;
}

export async function saveEmploymentRecord(
  profileId: string,
  record: EmploymentRecord,
  key: CryptoKey,
): Promise<void> {
  if (!isEmploymentCollection([record], profileId))
    throw new Error("Employment data is invalid.");
