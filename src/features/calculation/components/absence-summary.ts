...
import { SKILLED_WORKER_DEPENDANT_PERIOD_RULE } from "../domain/dependant-qualifying-period-rule";
...
const ISSUE_MESSAGES: Record<QualifyingPeriodIssue, string> = {
  ...
  "dependant-partner-linkage-not-verified":
    "Confirm that every recorded dependant permission was linked to the same partner; UrbanFox does not store or verify that relationship history.",
};
...
export function renderAbsenceSummary(
  ...
  const isDependant = period.issues.includes(
    "dependant-partner-linkage-not-verified",
  );
  const sourceMap = new Map(
    [
      ...(isDependant
        ? SKILLED_WORKER_DEPENDANT_PERIOD_RULE.sources
